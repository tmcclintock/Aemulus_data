...
inbase = "/nfs/slac/g/ki/ki22/cosmo/tmcclint/test_NM_Z_data/reduced_mf_data/TestBox%03d-%03d_reduced/TestBox%03d-%0\
3d_reduced_Z%d.list"

outbase = "/nfs/slac/g/ki/ki22/cosmo/tmcclint/NEWTESTMF/TestBox%03d-%03d/"
...
def run(box, real, snap):
    ...
    try:
        indata = pd.read_csv(inbase%(box, real, box, real, snap), dtype='float64', header=0, delim_whitespace=True)
        ...
        return
    os.system("mkdir -p %s"%(outbase%(box,real)))
    ...
    np.savetxt(outbase%(box,real)+"/TestBox%03d-%03d_Z%d.txt"%(box,real, snap), out, header = "logM_lo logM_hi N Mave")
    np.savetxt(outbase%(box,real)+"/TestBox%03d-%03d_Z%d_cov.txt"%(box,real, snap), C)
